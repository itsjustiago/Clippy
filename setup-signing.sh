#!/bin/bash
# Cria (uma vez) um certificado self-signed estável para assinar a Clippy.
# Assim a permissão de Acessibilidade cola de vez e sobrevive a recompilações.
# Usa uma keychain dedicada com password própria — não precisa da tua password de login.
set -euo pipefail
...
openssl pkcs12 -export -inkey "$WORK/key.pem" -in "$WORK/cert.pem" \
  -name "$IDENTITY" -out "$WORK/id.p12" -passout pass:clippy >/dev/null 2>&1
...
security import "$WORK/id.p12" -k "$KEYCHAIN" -P clippy -A -T /usr/bin/codesign >/dev/null 2>&1
