...
final class UpdateController: NSObject {
    ...
    func start(_ info: UpdateInfo) {
        ...
        model.status = "A transferir a Clippy \(info.version)…"
        model.failed = false
        showWindow()
    ...
    private func install(downloaded tmp: URL) {
        ...
        let work = fm.temporaryDirectory.appendingPathComponent("ClippyUpdate-\(UUID().uuidString)")
        let zip = work.appendingPathComponent("Clippy.zip")
